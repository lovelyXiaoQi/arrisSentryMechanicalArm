...
三条自动化装填通路共用同一套门禁（等级序列 + 单一等级库存 + `magazineSize × 5` 容量）：

- **普通动力臂**：识别哨戒臂上方 1.5 格的交互点（`registerArmPoint`），可存可取
- **机械动力漏斗/溜槽**（安山/黄铜漏斗、溜槽）：`registerFunnelContainer` 容器协议，可存可取
- **原版漏斗**：出口指向哨戒臂的相邻漏斗（正上方或四侧），按原版 0.4 秒/发抽取，仅存入
