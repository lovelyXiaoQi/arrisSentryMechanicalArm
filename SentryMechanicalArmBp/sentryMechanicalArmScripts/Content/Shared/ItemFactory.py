...
class ItemFactory(object):
    class __UserDataFactory(object):
        # ArrisCustomData 编码 magic 前缀
        # 用 base64 包装 pickle 字节，再加前缀，保证：
        #   1. 整个字符串是纯 ASCII —— 引擎网络同步 __type__=STRING 时不会因 UTF-8 编码异常崩溃
        #   2. 前缀让解码器可以快速识别 —— 无需 try/except 兜底损坏数据
        _PICKLE_PREFIX = "__pk__:"

        @staticmethod
        def __encodeCustomDataValue(value):
            """把任意 Python 值编码为 ASCII 安全字符串（base64 包装 pickle）"""
            pickled = pickle.dumps(value)
            encoded = base64.b64encode(pickled)
            # Py2: base64 返回 str (bytes)，isinstance(encoded, str) 为 True，保持 str
            # Py3: base64 返回 bytes，需要 decode 为 str
            # 注意：不能用 isinstance(encoded, bytes) 判断，因为 Py2 中 bytes == str
            # 会错误地把 str 转成 unicode，导致 NBTTransformer.transform 无法识别 → 返回 None
            if not isinstance(encoded, str):
                encoded = encoded.decode("ascii")
            return ItemFactory._ItemFactory__UserDataFactory._PICKLE_PREFIX + encoded

        @staticmethod
        def __decodeCustomDataValue(value):
            """
            解码自定义数据值。

            分支:
                - 非字符串类型：原始值（从 NBT unwrap 出来的 int/float/bool/list/dict），原样返回
                - 带 _PICKLE_PREFIX 的字符串：走 base64 + pickle 解码
                - 其他字符串：历史遗留或裸字符串，原样返回
            """
            if not isinstance(value, (str, unicode)):
                return value
            if isinstance(value, unicode):
                value = value.encode("ascii", "ignore")
            prefix = ItemFactory._ItemFactory__UserDataFactory._PICKLE_PREFIX
            if value.startswith(prefix):
                return pickle.loads(base64.b64decode(value[len(prefix):]))
            return value
        ...
        def __init__(self, itemDict=None):
            ...
            # ------- display 层 -------
            self.__display = (
                self.__userDataDict.get("display", {}) if self.__userDataDict else {}
            )  # type: dict
            self.__lore = self.__display.get("Lore", None)  # type: list or None
            self.__name = self.__display.get("Name", None)  # type: str or None
            # ---------- ench 层 ----------
            self.__ench = (
                self.__userDataDict.get("ench", None) if self.__userDataDict else None
            )  # type: list or None
            # ---------- trim 层 ----------
            self.__trim = (
                self.__userDataDict.get("Trim", None) if self.__userDataDict else None
            )  # type: dict or None
            ...
            # --------- repairCost 层 ---------
            self.__repairCost = (
                self.__userDataDict.get("RepairCost", None)
                if self.__userDataDict
                else None
            )  # type: dict or None
            # --------- extraId 层 ---------
            self.__extraId = (
                self.__userDataDict.get("ItemExtraID", None)
                if self.__userDataDict
                else None
            )  # type: str or None
            # --------- customTips 层 ---------
            self.__customTips = (
                self.__userDataDict.get("ItemCustomTips", None)
                if self.__userDataDict
                else None
            )  # type: str or None
            # ---------- customData 层 ----------
            self.__customData = (
                self.__userDataDict.get("ArrisCustomData", None)
                if self.__userDataDict
                else None
            )  # type: dict
        ...
        def __enchToNBTdata(enchData):
            ...
            if isinstance(enchId, int):
                return {
                    "id": NBTTransformer.transform(
                        enchId, NBTTransformer.DataType.SHORT
                    ),
                    "lvl": NBTTransformer.transform(
                        level, NBTTransformer.DataType.SHORT
                    ),
                }
            else:
                return {
        ...
        def setEnchantments(self, enchList):
            ...
            """
            dataList = [self.__enchToNBTdata(ench) for ench in enchList]
            self.__ench = dataList
            return self
        ...
        def addEnchantment(self, enchData):
            ...
            # 判断是否已经存在相同的附魔
            for ench in self.__ench:
                if isinstance(enchData.id, int):
                    if ench["id"]["__value__"] == enchData.id:
                        ench["lvl"]["__value__"] = enchData.level
                        return self
                else:
                    if ench["modEnchant"]["__value__"] == enchData.id:
                        ench["lvl"]["__value__"] = enchData.level
                        return self
            # 如果不存在相同的附魔，则添加新的附魔
        ...
        def setRepairCost(self, repairCost):
            self.__repairCost = (
                None if repairCost == 0 else NBTTransformer.transform(repairCost)
            )
            return self
        ...
        def setCustomData(self, data):
            self.__customData = NBTTransformer.transform(
                {key: self.__encodeCustomDataValue(value) for key, value in data.items()}
            )
            return self
        ...
        def addCustomData(self, data):
            if not self.__customData:
                self.__customData = {}
            encoded = {key: self.__encodeCustomDataValue(value) for key, value in data.items()}
            self.__customData.update(NBTTransformer.transform(encoded))
            return self
        ...
        def build(self):
            ...
            if self.__customData:
                userData["ArrisCustomData"] = self.__customData
            else:
                userData.pop("ArrisCustomData", None)
        ...
        def getCustomData(self):
            ...
            for key, value in rawData.items():
                data[key] = self.__decodeCustomDataValue(value)
    ...
    def __init__(self, itemDict=None):
        self.__itemDict = (
            itemDict
            if itemDict
            else {"newItemName": "minecraft:air", "newAuxValue": 0, "count": 1}
        )
