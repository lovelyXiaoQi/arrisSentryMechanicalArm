...
- **EP+ 子弹等级**：`bulletType` 只存基础弹名（gun data.useBullet）；库存实际弹种在 `reserveBulletType`（同时只存一种），弹匣逐发等级在 `magazineBulletList`（EP bullet_list 数字串：每位 = 弹药序列下标，降序排列，末尾先打 → 高级弹优先）。补弹统一走 `_refillMagazine`，射击按末位数字还原弹种，伤害 = 基础 × `BULLET_DATA['danger']` × `EpCompat.armorDamageFactor`（对锁定目标按 `entityArmor.ZOMBIE_ARMOR` 枪械护甲 + 枪 `PercentArmorPenetration`/`FlatArmorPenetration` + 子弹 `armor_penetration` 折算，`1 − 有效护甲/200`——**等级差距主要在这里**，各等级平射 danger 几乎相同且 lv1=1.1 最高是 EP 原设定）。`ARMOR_LEVEL` 部位甲耐久与破甲视觉是 EP 内部状态，不模拟。**不要绕过 `_refillMagazine` 直接改 `currentMagazine`**，数字串会和弹匣数失同步。
